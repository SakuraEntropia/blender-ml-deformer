...
class PSD_OT_sync_bones(bpy.types.Operator):
    bl_idname = "psd.sync_bones"
    ...
    def execute(self, context):
        n = bridge.sync_bones(context.scene.psd)
        self.report({"INFO"}, "Loaded %d bones" % n)
        return {"FINISHED"}


class PSD_OT_sync_curve_inputs(bpy.types.Operator):
    bl_idname = "psd.sync_curve_inputs"
    bl_label = "Load Shape Keys"
    bl_description = "List the mesh shape keys as curve inputs"

    def execute(self, context):
        n = bridge.sync_curve_inputs(context.scene.psd)
        self.report({"INFO"}, "Loaded %d shape keys" % n)
        return {"FINISHED"}


class PSD_OT_sync_morph_targets(bpy.types.Operator):
    bl_idname = "psd.sync_morph_targets"
    bl_label = "Load Shape Keys"
    bl_description = "List the mesh shape keys as morph targets (Neural model)"

    def execute(self, context):
        n = bridge.sync_morph_targets(context.scene.psd)
        self.report({"INFO"}, "Loaded %d shape keys" % n)
        return {"FINISHED"}


class PSD_OT_bones_remove(bpy.types.Operator):
    bl_idname = "psd.bones_remove"
...
class PSD_OT_key_remove(bpy.types.Operator):
    bl_idname = "psd.key_remove"
    bl_label = "Remove Key"
...
class PSD_OT_clip_add(bpy.types.Operator):
    bl_idname = "psd.clip_add"
...
class PSD_OT_clip_add_all(bpy.types.Operator):
    bl_idname = "psd.clip_add_all"
...
class PSD_OT_clip_remove(bpy.types.Operator):
    bl_idname = "psd.clip_remove"
...
class PSD_ProgressOperator(bpy.types.Operator):
...
class PSD_OT_generate_training_data(PSD_ProgressOperator):
    bl_idname = "psd.generate_training_data"
    bl_label = "Generate Training Data"

    def _make_generator(self, context):
        return train_ops.generate_training_data_iter(context.scene.psd)

    def _on_done(self, context):
        self.report({"INFO"}, "Generated %d training frames"
                    % context.scene.psd.num_training_frames)


class PSD_OT_train(PSD_ProgressOperator):
    bl_idname = "psd.train"
    bl_label = "Train Model"

    def _make_generator(self, context):
        return train_ops.train_model_iter(context.scene.psd)


class PSD_OT_bake_shape_keys(PSD_ProgressOperator):
    bl_idname = "psd.bake_shape_keys"
    bl_label = "Bake Shape Keys"

    def _make_generator(self, context):
        return train_ops.bake_shape_keys_iter(context.scene.psd)


class PSD_OT_export_engine_nmn(PSD_ProgressOperator):
    bl_idname = "psd.export_engine_nmn"
    ...
    def invoke(self, context, event):
        base = context.scene.psd.model_dir or (os.path.dirname(bpy.data.filepath)
                                               if bpy.data.filepath else "")
        self.filepath = os.path.join(base, (context.scene.psd.model_name
                                            or "posedeformer") + ".nmn")
    ...
    def _make_generator(self, context):
        ...
        return ue_ops.export_nmn_iter(context.scene.psd, self.filepath)
...
class PSD_OT_create_preview_proxy(bpy.types.Operator):
    bl_idname = "psd.create_preview_proxy"
...
class PSD_OT_refresh_preview(bpy.types.Operator):
    bl_idname = "psd.refresh_preview"
...
class PSD_OT_export_model(bpy.types.Operator):
    bl_idname = "psd.export_model"
...
class PSD_OT_import_model(bpy.types.Operator):
    bl_idname = "psd.import_model"
...
class PSD_OT_import_engine_nmn(bpy.types.Operator):
    bl_idname = "psd.import_engine_nmn"
    ...
    def execute(self, context):
        try:
            name = ue_ops.import_nmn(context.scene.psd, self.filepath)
...
class PSD_OT_import_engine_onnx(bpy.types.Operator):
    bl_idname = "psd.import_engine_onnx"
    ...
    def execute(self, context):
        try:
            name = ue_ops.import_onnx(context.scene.psd, self.filepath)
...
class PSD_OT_clear(bpy.types.Operator):
    bl_idname = "psd.clear"
    ...
    def execute(self, context):
        ...
        for obj in [o for o in context.scene.objects
                    if o.name.endswith(("_PSDPreview", "_PSDBaked"))]:
            bpy.data.objects.remove(obj, do_unlink=True)
        self.report({"INFO"}, "Cleared PoseDeformer state")
        return {"FINISHED"}


classes = (
    PSD_OT_sync_bones,
    PSD_OT_sync_curve_inputs,
    PSD_OT_sync_morph_targets,
    PSD_OT_bones_remove,
    PSD_OT_key_remove,
    PSD_OT_clip_add,
    PSD_OT_clip_add_all,
    PSD_OT_clip_remove,
    PSD_OT_generate_training_data,
    PSD_OT_train,
    PSD_OT_bake_shape_keys,
    PSD_OT_export_engine_nmn,
    PSD_OT_create_preview_proxy,
    PSD_OT_refresh_preview,
    PSD_OT_export_model,
    PSD_OT_import_model,
    PSD_OT_import_engine_nmn,
    PSD_OT_import_engine_onnx,
    PSD_OT_clear,
)
